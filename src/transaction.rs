...
impl Transaction {
    ...
    pub fn acquire_lock(&self, key: LockKey, mode: LockMode) -> Result<()> {
        ...
        loop {
            match self.lock_manager.try_acquire(self.id, self.priority, key.clone(), mode)? {
                LockResult::Granted => {
    ...
    pub fn write(&self, table: &str, row_id: u64, values: Vec<crate::types::Value>) -> Result<()> {
        ...
        // Write to storage
        self.storage.with_table_mut(table, |t| {
            t.update(row_id, values)
        })
    }
    ...
    pub fn insert(&self, table: &str, values: Vec<crate::types::Value>) -> Result<u64> {
        ...
        let row_id = self.storage.with_table_mut(table, |t| {
            t.insert(values)
        })?;
    ...
    pub fn scan(&self, table: &str, start: Option<u64>, end: Option<u64>) -> Result<Vec<(u64, Vec<crate::types::Value>)>> {
        if !self.is_active() {
            return Err(Error::TransactionNotActive(self.id));
        }
...
mod tests {
    ...
    fn test_concurrent_transactions() {
        ...
        let result = tx2.write("accounts", 1, vec![
            Value::Integer(1),
            Value::Integer(200),
        ]);
