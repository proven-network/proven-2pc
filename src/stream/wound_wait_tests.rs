...
mod tests {
    ...
    async fn test_basic_wound_older_wounds_younger() {
        ...
        for (i, sql) in setup_sql.iter().enumerate() {
            let msg = create_message(
                Some(SqlOperation::Execute { sql: sql.to_string() }),
        ...
        assert!(wounded_response.is_some(), "Younger should receive wounded notification");
    ...
    async fn test_wound_with_multiple_locks() {
        ...
        let responses = mock_channel.get_responses();
        let older_response = responses.iter()
        ...
        let new_response = all_responses.iter()
    ...
    async fn test_wound_preserves_determinism() {
        ...
        assert_eq!(results1.len(), results2.len(), "Should have same number of results");
        ...
        for (i, (r1, r2)) in results1.iter().zip(results2.iter()).enumerate() {
            ...
            assert_eq!(
                ...
                "Response type mismatch at position {}", i
            );
        }
    }
    ...
    async fn run_wound_sequence() -> Vec<(String, String, SqlResponse)> {
        ...
        let operations = vec![
            ("txn_runtime1_1000000000", "CREATE TABLE test (id INT PRIMARY KEY, value INT)", true),
            ("txn_runtime1_1000000001", "INSERT INTO test (id, value) VALUES (1, 100)", true),
            ("txn_runtime1_3000000000", "UPDATE test SET value = 300 WHERE id = 1", false),
            ("txn_runtime1_2000000000", "UPDATE test SET value = 200 WHERE id = 1", false),
            ("txn_runtime1_4000000000", "UPDATE test SET value = 400 WHERE id = 1", false),
        ];
    ...
    async fn test_wound_clears_wait_graph() {
        ...
        // Verify T2 succeeded
        let responses = mock_channel.get_responses();
        let t2_response = responses.iter()
            .filter(|(c, _, _)| c == "coord_t2")
            .last();
        assert!(matches!(
            t2_response,
            Some((_, _, SqlResponse::ExecuteResult { .. }))
