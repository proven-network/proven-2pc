...
pub struct DeferredOperation {
    /// The original SQL operation
    pub operation: SqlOperation,
    /// The original SQL string (for re-execution)
    pub sql: String,
    /// Coordinator to send response to
    pub coordinator_id: String,
    /// Lock that was requested
    pub lock_requested: (LockKey, LockMode),
...
impl DeferredOperationsManager {
    ...
    /// Get all transactions with deferred operations
    pub fn all_deferred_transactions(&self) -> Vec<HlcTimestamp> {
        let mut txs: Vec<_> = self.operations.keys().copied().collect();
        txs.sort(); // Deterministic order
        txs
    }

    /// Clear all deferred operations (for testing or reset)
    pub fn clear(&mut self) {
        self.operations.clear();
        self.wait_graph.clear();
        self.reverse_wait_graph.clear();
    }
}
