//! STDEV_DISTINCT (Standard Deviation with DISTINCT) aggregate function
...
pub struct StdevDistinctFunction;

impl Function for StdevDistinctFunction {
    ...
    fn validate(&self, arg_types: &[DataType]) -> Result<DataType> {
        if arg_types.len() != 1 {
            return Err(Error::ExecutionError(format!(
                "STDEV_DISTINCT takes exactly 1 argument, got {}",
                arg_types.len()
            )));
        }
    ...
    fn execute(&self, args: &[Value], _context: &TransactionContext) -> Result<Value> {
        ...
        if args.len() != 1 {
            return Err(Error::ExecutionError(
                "STDEV_DISTINCT takes exactly 1 argument".into(),
            ));
        }
...
/// Register the STDEV_DISTINCT function
pub fn register(registry: &mut FunctionRegistry) {
    registry.register(Box::new(StdevDistinctFunction));
}
...
mod tests {
    ...
    #[test]
    fn test_stdev_distinct_signature() {
    ...
    #[test]
    fn test_stdev_distinct_validate() {
        let func = StdevDistinctFunction;
