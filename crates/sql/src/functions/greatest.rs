//! GREATEST function - returns the greatest value from a list of arguments
...
impl Function for GreatestFunction {
    ...
    fn validate(&self, arg_types: &[DataType]) -> Result<DataType> {
        ...
        if matches!(first_type, DataType::Null) {
            return Err(Error::ExecutionError(
                "GREATEST does not accept NULL arguments".into(),
            ));
        }
        ...
        for arg_type in &arg_types[1..] {
            ...
            if arg_type != base_type {
                return Err(Error::ExecutionError(format!(
                    "GREATEST arguments must be of the same type, got {:?} and {:?}",
                    first_type, arg_type
                )));
            }
        }
    ...
    fn execute(&self, args: &[Value], _context: &TransactionContext) -> Result<Value> {
        ...
        for arg in &args[1..] {
            ...
            if compare_values(arg, max_value)? {
                max_value = arg;
            }
        }
...
fn compare_values(a: &Value, b: &Value) -> Result<bool> {
    match (a, b) {
        ...
        _ => Err(Error::ExecutionError(
            "GREATEST arguments are not comparable".into(),
        )),
    }
}

/// Register the GREATEST function
...
mod tests {
    ...
    fn test_greatest_signature() {
        ...
        assert_eq!(sig.name, "GREATEST");
        assert!(!sig.is_aggregate);
    }

    #[test]
    fn test_greatest_validate() {
        let func = GreatestFunction;
    ...
    fn test_greatest_execute() {
        ...
        // Integers
        let result = func
            .execute(&[Value::I64(1), Value::I64(6), Value::I64(3)], &context)
            .unwrap();
        ...
        let result = func
            ...
            .unwrap();
        assert_eq!(result, Value::string("cherry"));
