...
/// DEDUP function - removes duplicate values from a list
pub struct DedupFunction;

impl Function for DedupFunction {
    ...
    fn validate(&self, arg_types: &[DataType]) -> Result<DataType> {
        if arg_types.len() != 1 {
            return Err(Error::ExecutionError(
                "DEDUP takes exactly 1 argument".into(),
            ));
        }
...
pub fn register(registry: &mut FunctionRegistry) {
    registry.register(Box::new(DedupFunction));
}
