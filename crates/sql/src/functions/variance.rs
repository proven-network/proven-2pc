...
impl Function for VarianceFunction {
    ...
    fn validate(&self, arg_types: &[DataType]) -> Result<DataType> {
        if arg_types.len() != 1 {
            return Err(Error::ExecutionError(format!(
                "VARIANCE takes exactly 1 argument, got {}",
                arg_types.len()
            )));
        }
    ...
    fn execute(&self, args: &[Value], _context: &TransactionContext) -> Result<Value> {
        ...
        if args.len() != 1 {
            return Err(Error::ExecutionError(
                "VARIANCE takes exactly 1 argument".into(),
            ));
        }
...
/// Register the VARIANCE function
pub fn register(registry: &mut FunctionRegistry) {
    registry.register(Box::new(VarianceFunction));
}
...
mod tests {
    ...
    #[test]
    fn test_variance_signature() {
        let func = VarianceFunction;
        let sig = func.signature();
        assert_eq!(sig.name, "VARIANCE");
        assert!(sig.is_aggregate);
    }

    #[test]
    fn test_variance_validate() {
        let func = VarianceFunction;
