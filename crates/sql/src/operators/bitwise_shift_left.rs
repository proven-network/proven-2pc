//! Bitwise left shift operator implementation
...
pub struct BitwiseShiftLeftOperator;

impl BinaryOperator for BitwiseShiftLeftOperator {
    ...
    fn execute(&self, left: &Value, right: &Value) -> Result<Value> {
        ...
        Ok(match left {
            I8(n) => {
                ...
                I8(n.wrapping_shl(shift_amount))
            }
            I16(n) => {
                if shift_amount >= 16 {
                    return Err(Error::InvalidValue(format!(
                ...
                I16(n.wrapping_shl(shift_amount))
            }
            I32(n) => {
                if shift_amount >= 32 {
                    return Err(Error::InvalidValue(format!(
                ...
                I32(n.wrapping_shl(shift_amount))
            }
            I64(n) => {
                if shift_amount >= 64 {
                    return Err(Error::InvalidValue(format!(
                ...
                I64(n.wrapping_shl(shift_amount))
            }
            I128(n) => {
                if shift_amount >= 128 {
                    return Err(Error::InvalidValue(format!(
                ...
                I128(n.wrapping_shl(shift_amount))
            }
            U8(n) => {
                if shift_amount >= 8 {
                    return Err(Error::InvalidValue(format!(
                ...
                U8(n.wrapping_shl(shift_amount))
            }
            U16(n) => {
                if shift_amount >= 16 {
                    return Err(Error::InvalidValue(format!(
                ...
                U16(n.wrapping_shl(shift_amount))
            }
            U32(n) => {
                if shift_amount >= 32 {
                    return Err(Error::InvalidValue(format!(
                ...
                U32(n.wrapping_shl(shift_amount))
            }
            U64(n) => {
                if shift_amount >= 64 {
                    return Err(Error::InvalidValue(format!(
                ...
                U64(n.wrapping_shl(shift_amount))
            }
            U128(n) => {
                if shift_amount >= 128 {
                    return Err(Error::InvalidValue(format!(
                ...
                U128(n.wrapping_shl(shift_amount))
            }
            _ => {
                return Err(Error::InvalidOperation(format!(
                    "Cannot perform {} on {} and {}",
