...
pub trait DmlParser: TokenHelper {
    ...
    fn parse_from_table(&mut self) -> Result<FromClause> {
        ...
        if matches!(self.peek()?, Some(Token::OpenParen)) {
            ...
            // Check if this is UNNEST(array_expr) table-valued function
            if name.to_uppercase() == "UNNEST" && matches!(self.peek()?, Some(Token::OpenParen)) {
                self.next()?; // consume opening paren
                ...
                return Ok(FromClause::Unnest { array, alias });
            }
