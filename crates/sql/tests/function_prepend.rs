...
fn setup_prepend_table(ctx: &mut common::TestContext) {
...
fn test_create_table_with_list_and_elements() {
    ...
    ctx.exec("CREATE TABLE Prepend (id INTEGER, items LIST, element INTEGER, element2 TEXT)");
...
fn test_prepend_integer_to_list() {
    ...
    if let Value::List(list) = result_value {
        ...
        assert_eq!(list[1], Value::I64(1), "Second element should be 1");
        assert_eq!(list[2], Value::I64(2), "Third element should be 2");
        assert_eq!(list[3], Value::I64(3), "Fourth element should be 3");
    } else {
...
fn test_prepend_text_to_list() {
    ...
    let results = ctx.query("SELECT PREPEND(element2, items) as myprepend FROM Prepend");
    assert_eq!(results.len(), 1);

    let result_value = results[0].get("myprepend").unwrap();
    if let Value::List(list) = result_value {
        assert_eq!(list.len(), 4, "Should have 4 elements");
        // Check the prepended string and original values
        assert_eq!(
            list[0],
...
fn test_create_table_for_prepend_operation() {
    ...
    ctx.exec("CREATE TABLE Foo (elements LIST)");
...
fn test_insert_with_prepend_function() {
    ...
    ctx.exec("CREATE TABLE Foo (elements LIST)");
    ctx.exec("INSERT INTO Foo VALUES (PREPEND(0, CAST('[1, 2, 3]' AS LIST)))");
...
fn test_select_prepended_elements() {
    ...
    if let Value::List(list) = result_value {
        assert_eq!(list.len(), 4, "Should have 4 elements");
        assert_eq!(list[0], Value::I64(0), "First element should be 0");
        assert_eq!(list[1], Value::I64(1), "Second element should be 1");
        assert_eq!(list[2], Value::I64(2), "Third element should be 2");
        assert_eq!(list[3], Value::I64(3), "Fourth element should be 3");
    } else {
...
fn test_prepend_mixed_types() {
    ...
    ctx.exec("CREATE TABLE MixedTest (id INTEGER, items LIST, str_val TEXT)");
    ctx.exec("INSERT INTO MixedTest VALUES (1, '[1, 2, 3]', 'Hello')");

    // Test prepending a string to an integer list (creates mixed-type list)
    let results = ctx.query("SELECT PREPEND(str_val, items) as result FROM MixedTest");
    assert_eq!(results.len(), 1);

    let result_value = results[0].get("result").unwrap();
    if let Value::List(list) = result_value {
        assert!(!list.is_empty(), "Should have at least 1 element");
    } else {
        panic!("Expected List value, got: {:?}", result_value);
    }
...
fn test_prepend_with_cast() {
    ...
    let results = ctx.query("SELECT PREPEND(0, CAST('[1, 2, 3]' AS LIST)) as result");
    assert_eq!(results.len(), 1);
