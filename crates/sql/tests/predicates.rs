...
fn create_engine() -> AutoBatchEngine<SqlTransactionEngine> {
    AutoBatchEngine::new(SqlTransactionEngine::new(SqlStorageConfig::default()))
}
...
fn test_predicate_crash_recovery() {
    ...
    {
        let mut engine = AutoBatchEngine::new(SqlTransactionEngine::new(
            persistent_storage_config(data_path.clone()),
        ));
    ...
    // Phase 2: Restart engine and verify predicates were recovered
    {
        ...
        engine.commit(timestamp(2));

        engine.abort(timestamp(3));
