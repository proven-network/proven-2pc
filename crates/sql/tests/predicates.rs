...
fn test_predicate_crash_recovery() {
    ...
    // Phase 2: Restart engine and verify predicates were recovered
    {
        ...
        let mut batch = engine.start_batch();
        engine.commit(&mut batch, timestamp(2));
        engine.commit_batch(batch, next_log_index());

        let mut batch = engine.start_batch();
        engine.abort(&mut batch, timestamp(3));
        engine.commit_batch(batch, next_log_index());
...
fn test_predicate_recovery_with_multiple_operations() {
    ...
    // Phase 2: Verify all predicates recovered
    {
        ...
        // Clean up
        let mut batch = engine.start_batch();
        engine.abort(&mut batch, timestamp(2));
        engine.commit_batch(batch, next_log_index());
