...
fn test_variance_with_null_values() {
    ...
    if let Some(num_str) = var_val.strip_prefix("F64(").and_then(|s| s.strip_suffix(")")) {
...
fn test_variance_multiple_columns() {
    ...
    if let Some(num_str) = var_id.strip_prefix("F64(").and_then(|s| s.strip_suffix(")")) {
        let val: f64 = num_str.parse().unwrap();
        assert!((val - 4.666667).abs() < 0.01, "Expected ~4.667, got {}", val);
    } else {
    ...
    if let Some(num_str) = var_qty.strip_prefix("F64(").and_then(|s| s.strip_suffix(")")) {
        let val: f64 = num_str.parse().unwrap();
...
fn test_variance_comparison_distinct_vs_all() {
    ...
    if let Some(num_str) = var_all.strip_prefix("F64(").and_then(|s| s.strip_suffix(")")) {
        let val: f64 = num_str.parse().unwrap();
        // Sample variance = 96.57
        assert!((val - 96.57).abs() < 0.1, "Expected ~96.57 for VARIANCE(quantity), got {}", val);
    } else {
    ...
    if let Some(num_str) = var_distinct.strip_prefix("F64(").and_then(|s| s.strip_suffix(")")) {
        let val: f64 = num_str.parse().unwrap();
        assert!((val - 93.3).abs() < 0.1, "Expected ~93.3 for VARIANCE(DISTINCT quantity), got {}", val);
    } else {
        panic!("Expected F64 value for VARIANCE(DISTINCT quantity), got {}", var_distinct);
    }
...
fn test_variance_requires_multiple_values() {
    ...
    ctx.assert_query_contains("SELECT VARIANCE(value) FROM Item", "VARIANCE(value)", "Null");
